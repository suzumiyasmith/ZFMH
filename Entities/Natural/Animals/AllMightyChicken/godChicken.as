...
const int MAX_EGGS = 100; //maximum symultaneous eggs
const int MAX_CHICKENS = 5000;
...
void onInit(CBlob@ this)
{
	...
	vars.jumpForce.Set(20.0f, -20.0f);
...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (att !is null)
		{
			...
			if (b !is null)
			{
				...
				b.AddForce(Vec2f(0, -27));

				CBlob@[] foes;
				getBlobsByTag("player", foes);

				for (uint i=0; i<foes.size(); i++)
				{
					if (b.getTeamNum() != foes[i].getTeamNum())
					{
						foes[i].Damage(0.008f, b);
						if (foes[i].getHealth() < 0.f)
						{
							foes[i].server_Die();
						}
					}
				...
				timetodoom++;
				float close = Maths::Max(0.f, 400.f - (timetodoom*2.f));

				if (XORRandom(close/10) == 0)
				{
					Sound::Play("/ScaredChicken");
				}

				CBlob@ rek = server_CreateBlob("chicken", -1, Vec2f(this.getPosition().x + Maths::Sin(time/10.f)*close, this.getPosition().y + Maths::Cos(time/10.f)*close));
				if (rek !is null)
				{
					rek.server_Die();
				}
	...
	else if (!this.isOnGround())
	{
		...
		if (vel.y > 0.5f)
		{
			this.AddForce(Vec2f(0, 50));
		}
	}
	else if (g_lastSoundPlayedTime + 30 < getGameTime())
	{
		g_lastSoundPlayedTime =  getGameTime();
...
{
	attached.AddScript("HAILCHICKEN.as");
	CPlayer@ player = attached.getPlayer();
	if (player !is null)
		print(attached.getPlayer().getUsername() + " HAS SACRIFIED FOR THE CHICKEN LORD!");
	timetodoom = 0;
	attached.set_u8("knocked", 255);
}